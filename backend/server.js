...
// ─── Middlewares ──────────────────────────────────────────────────────────────
app.use(cors({
  origin: process.env.FRONTEND_URL || true, // true permite qualquer origem durante dev local
  credentials: true
}));
...
app.get('/api/me', (req, res) => {
  ...
  if (req.session.funcionario) {
    return res.json({ logado: true, tipo: 'funcionario', ...req.session.funcionario });
  }
